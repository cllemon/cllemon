import { useData, withBase, DefaultTheme } from "vitepress";
...
export function extractSocialLink(
  name: string,
  target?: DefaultTheme.SocialLink[]
) {
  return target?.filter((t) => t.icon === name)[0]?.link;
}
